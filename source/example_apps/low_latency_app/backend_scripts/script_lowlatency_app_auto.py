...
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
C:/Users/laure/AppData/Local/Programs/Python/Python37/python.exe c:/Users/laure/wirepas/wm-sdk/source/example_apps/low_latency_app/backend_scripts/script_lowlatency_app_auto.py -s 192.168.1.91 -p 1883 -u mqttmasteruser -pw y51JlJodoAKphnNB6uy7qusyM4C -fu
Copyright 2021 Wirepas Ltd. All Rights Reserved.
See file LICENSE.txt for full license details.
"""

import sys,time                  # to handle script exit process
...
class BackendShell(Cmd):
    ...
    def do_send_led_on_command(self, arg):
        ...
        payload = pack('<BBBBB', MSG_ID_LED_SET_STATE, led_state, LED_ID, 1,1)
        self._send_data_request_message(gw_id,
                                        sink_id,
                                        payload,
                                        dst_addr,
    ...
    def do_send_led_off_command(self, arg):
        ...
        # Create payload with format <message ID (1 byte)> <LED ID (1 byte)> <LED state (1 byte)>
        payload = pack('<BBBBB', MSG_ID_LED_SET_STATE,led_state, LED_ID,1,1 )   #        payload = pack('<BBB', MSG_ID_LED_SET_STATE, LED_ID, led_state)
        self._send_data_request_message(gw_id,
                                        sink_id,
                                        payload,
                                        dst_addr,
    ...
    def do_send_led_stop_command(self, arg):
        "Send message to set node(s) LED state off. Format: send_led_off_command <gateway ID> <sink ID> <destination address> "
        """
        Function to encode and send <LED set state> message to node with the given argument
        from shell.
        ...
        # Create payload with format <message ID (1 byte)> <LED ID (1 byte)> <LED state (1 byte)>
        payload = pack('<BBBBB', MSG_ID_LED_SET_STATE,2, LED_ID,0xc0,0xdb )   #        payload = pack('<BBB', MSG_ID_LED_SET_STATE, LED_ID, led_state)
        self._send_data_request_message(gw_id,
                                        sink_id,
                                        payload,
                                        dst_addr,
    ...
    def do_send_led_alea_command(self, arg):
        "Send message to set node(s) LED state off. Format: send_led_off_command <gateway ID> <sink ID> <destination address> "
        """
        Function to encode and send <LED set state> message to node with the given argument
        from shell.
        ...
        # Create payload with format <message ID (1 byte)> <LED ID (1 byte)> <LED state (1 byte)>
        payload = pack('<BBBBB', 0xc0,2, 0xdb,0xc0,0xdb )   #        payload = pack('<BBB', MSG_ID_LED_SET_STATE, LED_ID, led_state)
        self._send_data_request_message(gw_id,
                                        sink_id,
                                        payload,
                                        dst_addr,
...
def main():
    ...
    arg = "ExoticWirepasGW_2 sink1 0xffffffff"

    while(1):
        BackendShell(wni).do_send_led_off_command(arg)
        time.sleep(5)
        BackendShell(wni).do_send_led_stop_command(arg)
        time.sleep(3)



        BackendShell(wni).do_send_led_on_command(arg)
        time.sleep(5)

        BackendShell(wni).do_send_led_stop_command(arg)
        time.sleep(3)

        BackendShell(wni).do_send_led_alea_command(arg)
        time.sleep(3)



   # do_send_led_on_command()
